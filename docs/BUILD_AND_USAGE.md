...
`FluoParam_PhotonsPerSecond`, and `General_BackgroundPhotonsPerSec` are all expressed as rates
...
`SimType_DriftNmPerSec` is a stage-drift rate, in nm/sec, applied along X; Y drifts
...
`SimType_ResolutionSpacingsNm` property: a comma-separated list of nm values, editable
...
reproducibility (same `SimType_RandomSeed` + params -> byte-identical stack), and
