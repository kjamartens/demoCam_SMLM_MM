...
- `General_` -- FOV/binning/acquisition-mode/stack-playback plumbing, plus
  ...
  equivalent at all (`AcqMode`, `StackLength`, `GenerateStack`, etc.).
...
- `PSFParam_PsfImmersionIndex`, `PSFParam_PsfOversampling` (default 4, was 12 before step 5 --
  lowered so the out-of-the-box oversampled kernel stays small regardless
  of `PSFParam_PsfModel`, see the step 5 performance Gotcha), `PSFParam_PsfKernelHalfWidthPx`
...
- `PSFParam_PsfZernikeCoefficients` (string, 15 comma-separated values, default all-
...
- `PSFParam_PsfZernikePreset` -- convenience dropdown on top of
  `PSFParam_PsfZernikeCoefficients`: `None` (default) | `AstigmatismWeak` |
...
`CamParam_QuantumEfficiency`, `CamParam_DarkCurrentElectronsPerSec`, `CamParam_PixelGainStdPct`,
`CamParam_PixelReadNoiseStdPct` (the last two are relative pixel-to-pixel spread on
top of `CamParam_CameraGainPhotonsPerADU`/`CamParam_ReadNoiseElectrons`, sCMOS-style;
...
(0.85), `CamParam_DarkCurrentElectronsPerSec` (1.03), `CamParam_CameraGainPhotonsPerADU`
(0.25), and `CamParam_ReadNoiseElectrons` (1.2) match the Photometrics Kinetix22
sCMOS Sensitivity (CMS) mode datasheet; `CamParam_PixelGainStdPct`/
`CamParam_PixelReadNoiseStdPct` default to 5%/20% as estimates, since Photometrics
...
- **`PSFParam_PsfKernelHalfWidthPx` is a minimum, not the actual value used** --
...
- **This model is meaningfully slower to (re)compute than the radially-
  ...
  planes, and `PSFParam_PsfOversampling`/`PSFParam_PsfKernelHalfWidthPx` can put the
  ...
  - `ComputePsfKernelCache` takes an optional `logCallback` (both
    ...
    `PSFParam_PsfKernelHalfWidthPx`/`PSFParam_PsfZRangeUm`/`PSFParam_PsfZStepUm`) whenever
  ...
  - `PlaneJob.process()`'s pupil arrays (`pupilRe`/`pupilIm`) are flattened
    ...
    signal at that size).
  - `PSFParam_PsfOversampling`/`PSFParam_PsfKernelHalfWidthPx` defaults were lowered to 4/16
  ...
  - Benchmarked (this dev machine, 12 logical cores, post all of the above):
    ...
    `PSFParam_PsfKernelHalfWidthPx`/`PSFParam_PsfZRangeUm` well past their new defaults)
...
`General_LabelingEfficiencyPct` (default 100), `SimType_StructureZRangeNm` (default
...
(`Nearest`|`Linear`|`Cubic`, default `Nearest` = today's exact box-average
behavior). `Linear`/`Cubic` sample the oversampled kernel plane at the
...
property (`Direct`|`ChirpZ`, default `Direct` = today's exact per-pixel
polar-quadrature sum), `GibsonLanniZernike`-only. `ChirpZ` reformulates the
